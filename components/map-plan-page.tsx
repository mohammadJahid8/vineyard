            ...
            <CollapsibleContent className='mt-3'>
              <div className='space-y-3 text-xs bg-gray-50 rounded-lg p-3'>
                ...
                <div className='border-t border-gray-200 pt-2 flex gap-2'>
                  ...
                  {/* {isVineyard && location.data.vivino_link && (
                    <a
                      href={location.data.vivino_link}
                      target='_blank'
                      rel='noopener noreferrer'
...
export default function MapViewPage() {
  ...
  const updateMapWithNewOrder = (newLocations: LocationItem[]) => {
    ...
    // Clear existing directions renderer and overlays
    if (directionsRenderer) {
      directionsRenderer.setMap(null);
    }

    // Clear any existing route overlays
    ...
    // Create new route with updated order
    if (newLocations.length > 1) {
      const directionsService = new window.google.maps.DirectionsService();

      if (newLocations.length === 2) {
        const request = {
          origin: { lat: newLocations[0].lat, lng: newLocations[0].lng },
          destination: { lat: newLocations[1].lat, lng: newLocations[1].lng },
        ...
        const waypoints = newLocations.slice(1, -1).map((location) => ({
        ...
        const request = {
          origin: { lat: newLocations[0].lat, lng: newLocations[0].lng },
          destination: {
            lat: newLocations[newLocations.length - 1].lat,
            lng: newLocations[newLocations.length - 1].lng,
          },
          waypoints: waypoints,
          travelMode: window.google.maps.TravelMode.DRIVING,
                    ...
                    <div className='flex items-center justify-center gap-2'>
                      ...
                      <span
                        className={`text-sm font-medium ${
                          getPlanTimeRemaining() === 'Expired'
                            ? 'text-red-600'
                            : getPlanTimeRemaining()?.includes('m left') &&
                              !getPlanTimeRemaining()?.includes('h')
                            ? 'text-amber-600'
                            : 'text-green-600'
                        }`}
                      >
                        {getPlanTimeRemaining() || 'Active'}
                      </span>
                    </div>
                  </div>
                )} */}
